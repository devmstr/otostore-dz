...
export function DebtMutateDrawer({
  open,
  onOpenChange,
  debt
}: DebtMutateDrawerProps) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const data = {
        type: formData.get('type') as 'CUSTOMER_DEBT' | 'SUPPLIER_LOAN',
        amount: Number(formData.get('amount')),
        entityId: formData.get('entityId')?.toString(),
        entityName: formData.get('entityName')?.toString(),
        dueDate: formData.get('dueDate')
          ? new Date(formData.get('dueDate') as string)
          : undefined,
        description: formData.get('description')?.toString()
      }
      await createDebtAction(data)
    ...
    <Drawer open={open} onOpenChange={onOpenChange}>
      <DrawerContent>
        <DrawerHeader>
          ...
          <DrawerDescription>
            Record a new debt or loan for a customer or supplier
          </DrawerDescription>
        </DrawerHeader>
